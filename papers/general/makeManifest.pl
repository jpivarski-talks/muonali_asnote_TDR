...
my $VERSION = sprintf "%d.%03d", q$Revision: 1.20 $ =~ /(\d+)/g;
my $verbose;
...
#
   ...
   close(FILE);
   # extract cvs info: exemplars-- (note that these are the real values for this file)
   #\RCS$Revision: 1.20 $
   #\RCS$Date: 2009/02/22 14:15:54 $
   #\RCS$Name:  $
   my $cvs_tag;
   # use concatenated patterns to avoid replacement by cvs
   my $pattern = "\\RCS\\\$"."Revision:\\s+(\\d+\\.\\d+)\\s+"."\\\$";
   m/$pattern/s;
   my $revision = $1;
   $pattern = "\\RCS\\\$"."Name:\\s+(\\S*)\\s+"."\\\$";
   if (m/$pattern/s)
   {
       $cvs_tag = $1;
   }
   ...
   {
       $cvs_tag = "REV-".$1;  # falls over to the revision if no tag
   }
   my $cvs_year;
   $pattern = "\\RCS\\\$"."Date:\\s+(\\d{4})";
   if ( m/$pattern/s )
   {
       $cvs_year = $1;
   }
   my $cvs_date = $cvs_year; # default value is just year
   $pattern = "\\RCS\\\$"."Date:\\s+(\\d{4}/\\d{2}/\\d{2})";
   if (m/$pattern/s) {$cvs_date = $1;}
   # extract metadata
...
{# Generate figure name
    my $outFile = undef;
    $newFig = "Figure_".&formatNumber($figNum+1);
